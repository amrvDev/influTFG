...
const cchangeLanguage = async (language) => {
  ...
  for (const ctextToChange of ctextsToChange) {
    ...
    if (ctextToChange.tagName === "INPUT" && ctextToChange.hasAttribute("placeholder")) {
      ctextToChange.placeholder = ctexts[csection][cvalue];
    } else {
  ...
  const submitValue = ctexts["registro"]["registrar"];
